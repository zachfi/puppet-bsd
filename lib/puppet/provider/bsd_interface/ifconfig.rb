...
Puppet::Type.type(:bsd_interface).provide(:ifconfig) do
  ...
  def self.instances
    ...
    begin
      ...
      PuppetX::BSD::Ifconfig.new(output).parse.each do |k, v|
        ...
        if !destroyable_interfaces.select { |i| k =~ /^#{i}/ }.empty?
  ...
  def flush
    ...
    if @property_hash[:destroyable]
      destroyable = if @property_hash[:destroyable] == :true
                      true
                    else
                      false
                    end
    else
      if !self.class.destroyables.select { |i| @property_hash[:name] =~ /^#{i}/ }.empty?
        destroyable = true
      else
        destroyable = false
      end
